function [iter, dec_seq, success_dec] = nb_ldpc_MV_SF2(LLR_2, vi,v0v1, nui, iter_max, mul_mat, add_mat, div_mat, combs, h)
...
while iter<iter_max
    ...
    for i = 1 : M
        ...
        for l = 1 :L1
            ...
            ii2 = R{i}(:,l)+1;
            for j1 = 1 : dc(i)
                if prod(eq([1:j1-1 j1+1:dc(i)]))==1
                    Wmn{i}(j1, ii2(j1)) = Wmn1{i}(j1, ii2(j1)) + v0;
                    Wn(idx1(j1), ii2(j1)) = Wn(idx1(j1), ii2(j1)) + v0;
                else
                    Wmn{i}(j1, ii2(j1)) = Wmn1{i}(j1, ii2(j1)) + v1;
